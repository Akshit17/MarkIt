...
img_canny = cv2.Canny(img_blur, 100, 200)                         # Edge detection on processed image
                 #  ( input_img, Lower Threshold, Upper threshold)
                    #  https://docs.opencv.org/3.4/da/d22/tutorial_py_canny.html

cv2.imshow('Edge detection', img_canny)

contours = cv2.findContours(img_canny, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)   # Find contours

img_contours = img.copy()
cv2.drawContours(img_contours, contours[0], -1, (0,255,0), 3)  #parameters are (image, contours,contour_color, contour_thickness)
